...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if($esValido == true) {
        if($password === '' && $confirmPassword === '') {
            $conn = new mysqli($servername, $mysql_username, $mysql_password, $dbname);
            $stmt = $conn->prepare("UPDATE usuarios SET nombre = ?, apellido = ?, username = ?, userType = ? WHERE id = ?");
            $stmt->bind_param("ssssi", $nombre, $apellido, $username, $userType, $idTS);
            $stmt->execute();
            ...
            if($esValido == true) {
                ...
                $stmt = $conn->prepare("UPDATE usuarios SET nombre = ?, apellido = ?, username = ?, password = ?, userType = ? WHERE id = ?");
                $stmt->bind_param("sssssi", $nombre, $apellido, $username, $hashedPassword, $userType, $idTS);
                $stmt->execute();
